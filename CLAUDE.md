...
# 先把 SDK 安装到一个 prefix（一次性，SDK 改了之后才需要重做）
cmake --install /path/to/wujihandpy/build --prefix /path/to/wujihandpy/install
# 然后让 colcon 用 find_package(wujihandcpp CONFIG) 找到它
colcon build --symlink-install \
    --cmake-args -DCMAKE_PREFIX_PATH=/path/to/wujihandpy/install
...
cd /path/to/wujihandpy
cmake -B build -DCMAKE_BUILD_TYPE=Release -DBUILD_STATIC_WUJIHANDCPP=ON
cmake --build build -j$(nproc)
cmake --install build --prefix $PWD/install

# 2. 在 wujihandros2 仓 build 时把 prefix 传给 colcon
cd /path/to/wujihandros2
colcon build --cmake-args -DCMAKE_PREFIX_PATH=/path/to/wujihandpy/install
```

不传 `CMAKE_PREFIX_PATH` 会报错 `Could not find a package configuration file provided by "wujihandcpp"`。

`wujihandcpp::wujihandcpp` 这个 imported target 会自动传递 spdlog / Threads / usb-1.0 链接，不需要再手动 link。
